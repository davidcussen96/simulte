...
void LteRlcUmRealisticD2D::initialize(int stage)
{
    if (stage == 0)
    {
        ...
        {
            nodeType_ = UE;
            if (macType.compare("LteMacUeRealisticD2D") != 0 and macType.compare("LteMacVueV2X") != 0)
                throw cRuntimeError("LteRlcUmRealisticD2D::initialize - %s module found, must be LteMacUeRealisticD2D or LteMacVueV2X. Aborting", macType.c_str());
