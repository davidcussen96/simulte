...
void LtePhyVueV2X::handleSelfMessage(cMessage *msg)
{
    ...
    {
        // Decode SCI's before TB's because we need the
        // MCS from the SCI to decode the TB.
        while (!v2xReceivedFramesSCI_.empty())
        {
            ...
            if (strcmp(frame->getName(), "SCI") == 0)
            {
                UserControlInfo* lteInfo =
                        check_and_cast<UserControlInfo*>(frame->removeControlInfo());
    ...
    {
        // Iterate through subchannels created from received
        // AirFrames and update the sensing window.
        bool noMatch;

        // For each SCI find the matching TB.
        // Add them together (RSRP, RSSI, RbMap, etc.)
    ...
    numAirFramesWithSCIsReceived_ = 0;
    numAirFramesWithSCIsNotReceived_ = 0;
    numAirFramesWithTBsReceived_ = 0;
    numAirFramesWithTBsNotReceived_ = 0;
    dist = 0;
...
{
    int pRsvpTxPrime = 0, prioRx = 0, k = 1, lSubch = 1, pRsvpRx,
    pStep = 100, thresAddition = 0, numSubch = numSubchannels, numCsrs,
    subchIndex, q = 1, msAgo = 1000, thIndex, riv, Th;
    bool wrapped;
    ...
    {
        Sa.clear();
        // Initialize set Sa. Its necessary to reinitialize Sa at
        // every iteration.
        for (int i = 0; i < 100; i++)
        {
            std::vector<Subchannel*> temp;
        ...
        wrapped = false;
        /*
         * Iterate through sensing window starting at pointerToEnd and
         * finishing at current. (REVERSE) - Special Condition: C=0, E=999.
         *
         */
        ...
        for (int frame = pointerToEnd; frame != current || !wrapped; frame--)
        {
            ...
            for (int channel = 0; channel < numSubchannels; channel++)
            {
                ...
                {
                    // Determine overlapping subframes which the
                    // current notSensed subframe.
                ...
                // Subchannel is not free.
                } else if (sensingWindow[frame][channel]->getIsFree() == 0)
                {
            ...
            {
                wrapped = true;
                // Special condition: If current=0 and pointerToEnd=999, then
                // we dont want to wraparound and reset frame to 999.
                if (current != 0) frame = 999;
                else frame = 1;
            }
        }
        thresAddition += 3;
    } while (numCsrs < 100);       // 0.2 * 500
    ...
    for (unsigned int x = 0; x < 100; x++)
    {
        for (unsigned int y = 0; y < numSubchannels; y++)
        {
            ...
            {
                // Subchannel has an rsrp and rssi value and is still free.
                if (subchannel->getRsrp() == 1)
                {
                    rssi = subchannel->getRssi();
                    rssiValues.push_back(make_tuple(rssi, x, y));
                } else {
                    // Subchannel is free, no airframe received.
    ...
    // While the number of CSRs available to send to the MAC layer < 80
    // continually add CSRs with the lowest RSSI values.
    bool b;
    if (rssiValues.size() > 0) b = true;
    else b = false;     // There are no CSRs in rssiValues.
    while (size < 100 && b)
    {
        // There are no CSRs in rssiValues.
        if (rssiValues.size() == 0) break;
...
void LtePhyVueV2X::handleAirFrame(cMessage* msg)
{
    ...
    if (strcmp(frame->getName(), "data frame") == 0 || strcmp(frame->getName(), "SCI") == 0)
    {
        if (v2xDecodingTimer_ == NULL)
        {
            ...
            v2xDecodingTimer_->setSchedulingPriority(9);
        ...
        if (updateSensingWindow_ == NULL)
        {
            ...
            updateSensingWindow_->setSchedulingPriority(10); // last thing to be performed in this TTI
...
{
    // This needs to contain RbMap
    UserControlInfo* lteInfo =
            check_and_cast<UserControlInfo*>(msg->removeControlInfo());
    ...
    {
        LteSchedulingGrantMode4* grant =
                check_and_cast<LteSchedulingGrantMode4*>(msg);
    ...
    {
        // Message received is a data packet. Store it and send it + SCI
        // when the next grant is received. If there was no data packet received
        // when a grant is received you still send an SCI. if the data packet is
        // null then just an SCI is sent.
...
void LtePhyVueV2X::storeAirFrame(LteAirFrame* newFrame)
{
    ...
    MacNodeId enbId = binder_->getNextHop(newInfo->getSourceId());
        ...
        for (const auto &myRemote : rbmap)
        {
            ...
            for ( const auto &myBand : myRemote.second ) {
                ...
                if (subchannelIndex == -1)
                {
                    ...
                    else if (band == 10) subchannelIndex = 1;
                    else if (band == 20) subchannelIndex = 2;
                    else if (band == 30) subchannelIndex = 3;
                    else if (band == 40) subchannelIndex = 4;
                }
    ...
    } else if (strcmp(newFrame->getName(), "data frame") == 0 )
    {
        ...
        for (const auto &myRemote : rbmap)
        {
            ...
            for ( const auto &myBand : myRemote.second ) {
                ...
                if (subchannelIndex == -1)
                {
                    if (band == 2) subchannelIndex = 0;
                    else if (band == 12) subchannelIndex = 1;
                    else if (band == 22) subchannelIndex = 2;
                    else if (band == 32) subchannelIndex = 3;
                    else if (band == 42) subchannelIndex = 4;
                }
                rsrp += rsrpVector.at(band);
                rssi += rssiVector.at(band);
            }
...
void LtePhyVueV2X::updatePointers()
{
    ...
    updatePointers_->setSchedulingPriority(8);
...
void LtePhyVueV2X::decodeAirFrame(LteAirFrame* frame, UserControlInfo* lteInfo)
{
    ...
    if (strcmp(frame->getName(), "SCI") == 0)
    {
        ...
        mcsMap[lteInfo->getSourceId()] = sciMcs;
        ...
        else
        {
            unsigned int mcs = 0;
            Coord myCoord = getCoord();
            std::vector<double> rsrpVector = check_and_cast<LteRealisticChannelModel*>(channelModel_)->getRSRP_D2D(frame, lteInfo, nodeId_, myCoord);
            result = check_and_cast<LteRealisticChannelModel*>(channelModel_)->error_D2D(frame, lteInfo, rsrpVector, mcs);
        }
        ...
        // No SCI for the TB.
        if (sciReceivedCorrectly[lteInfo->getSourceId()] == 0 && mcsMap.find(lteInfo->getSourceId()) == mcsMap.end())
        {
            return;
        }
